...
public:
	...
	ComPtr<ID3D11ShaderResourceView>	m_pNormalMapSRV[5];
	vector<CTexture*>					m_pNormalMapList;

	std::map <wstring, shared_ptr<MapObject>> m_MapObjList;
public:
	...
	shared_ptr<MapObject> SetUpMapObject(shared_ptr<CFbxObj>  obj, TVector3 scale , TVector3 Rotation , TVector3 Trasnfrom);
	void CheckDuplicationName(wstring& name);
